...
def add_cookies(driver,path):
    ...
    try:
        cookieID = driver.get_cookie('JSESSIONID')['value']
        cookieID =cookieID.split('.')[-1]
        cookies = pickle.load(open(path + "/cookies_"+cookieID+".pkl", "rb"))
        cookie_text_file=open('C:\\Users\\yvsobol3\\PycharmProjects\\Test\\user\\Session_info2.txt', 'w+')
        driver.delete_all_cookies()
        existing_cookies = []
        for cookie in cookies:
            if cookie['name'] not in list(existing_cookies):
                if (cookieID in cookie['value']) or cookie['name']!='JSESESSIONID':
                    cookie_text_file.write(str(cookie)+"\n")
                    driver.add_cookie(cookie)
                    existing_cookies.append(cookie['name'])
...
def login_with_password(driver):
    ...
    delete_cookies(driver, pathname)
    # cookieID = driver.get_cookie('JSESSIONID')
    # cookieP = driver.get_cookie('P')
    # cookieInfo = driver.get_cookie('cookieInfo')
    # cookieServer = driver.get_cookie('BIGipServermsk-pool-remedy-10.74.64.71-5555')
    ...
    if len(cookies)>4:
        for cookie in cookies:
            if cookie['name'] in ['JSESSIONID','P','cookieInfo','BIGipServermsk-pool-remedy-10.74.64.71-5555']:
                cookies_ID.append(cookie)
        driver.delete_all_cookies()
        if cookies_ID:
            driver.add_cookie(cookies_ID[0])
    # time.sleep(5)
        # driver.get(
        #     'http://remedy.msk.mts.ru/')
    ...
    try:
        ...
        try:
            ...
            # здесь нужно перехватить сообщение о неверной сессии или истекшем тайм ауте ARERR9201

            if '[9388]' in alert.text: #отрабатывается случай когда старый пароль не подходит
                print('Введите логин:')
                login = input("")
                print('Введите пароль:')
                password = input("")
...
def save_cookies(driver,pathname):
    cookieID = driver.get_cookie('JSESSIONID')['value'].split('.')[-1]
    pickle.dump(driver.get_cookies(), open(pathname + "/cookies_"+cookieID+".pkl", "wb"))

def delete_cookies(driver,pathname):
    try:
        cookieID = driver.get_cookie('JSESSIONID')['value'].split('.')[-1]
        if os.path.exists(pathname + "/cookies_"+cookieID+".pkl"):
            os.remove(pathname + "/cookies_"+cookieID+".pkl")
    except:
        pass
...
def get_performer_by_mail(mail,type_W):
    dicMail = {'stan.oksi@ya.ru': 'Власов Станислав Игоревич',
               ...
               'hpdefender@yandex.ru': 'Шпаков Илья Андреевич',
    ...
    dicGroup = {'mon23@mail.ru': 'ЮГ\Краснодар\ОРС\ТС',
                ...
                'lika_tyutina@bk.ru': 'ЮГ\Краснодар\ОРС\ТС'}
...
def get_work_number(driver):
    ...
    try:
        ...
        if len(el) > 0:
            ...
            work_number ='Не нашлось элемента arid_WIN_0_1'
    ...
    for i in range(1,5):
        ...
        for n, sub_klass in enumerate(klassificator):
            ...
            try:
                ...
                driver.find_element_by_xpath('//*[@id="reg_img_536870905"]').click()
                continue
            time.sleep(0.2)
        time.sleep(1)
...
def remedy_set_param(driver,**kwargs):
    ...
    while n<4:
        ...
        try:
            ...
            # time.sleep(10)
            if kwargs['initiator']:
                if ('Максименко' in kwargs['initiator']): #or ( 'Кузьмин' in kwargs['initiator']):  # защита от однофамильцев Максименко Олега и Кузьмина Саши
                    set_value(driver, "arid_WIN_0_540000008", kwargs['initiator'])
                    driver.switch_to.window(driver.window_handles[1])
                    elements = driver.find_elements_by_xpath('//*[@id="T536870926"]/tbody/tr/td[2]')
                    for el in elements:
                        if 'ведущий' in el.text:
                    ...
                    set_value(driver, "arid_WIN_0_540000008", kwargs['initiator'])
            if kwargs['incident']:
                set_value(driver, "arid_WIN_0_540000017", kwargs['incident'])
            # time.sleep(10)
            # даты
            ...
            ne=set_klassificator(driver,kwargs['klassificator'])
            if ne=='error':
                return 'Ошибка! Не удалось установить классификатор. Попробуйте заново.'


            if kwargs['opercontext']:
                set_value(driver, "arid_WIN_0_540000049", kwargs['opercontext'])
                set_value(driver, "arid_WIN_0_821600", 'нет проекта')
            ...
            try:
                if kwargs['impactS'] != 'Нет влияния на сервис абонентам в зоне действия СЭ.':
                    ...
                    time.sleep(2)
                    driver.find_element_by_xpath('//*[@id="WIN_0_536870907"]/div/div').click()
                    work_number = (driver.find_element_by_id('arid_WIN_0_1').get_attribute('value'))
                    if work_number == 'MSK':
                        time.sleep(10)
                        work_number = (driver.find_element_by_id('arid_WIN_0_1').get_attribute('value'))
            except Exception as ex:
...
def main(argv):
    pathname = 'C:\\Users\\yvsobol3\\PycharmProjects\\Test'
    # argv = ['C:\\Users\\yvsobol3\\PycharmProjects\\Test\\Remedy_web_1_0.py', '232596 переключение нагрузки ** days=5\r \n', 'sob.evg@yandex.ru', 'newwork !']

    try:
        if ('bs' in argv[3].lower()) or ('бс' in argv[3].lower()):
            performer, performerG, initiator = get_performer_by_mail(argv[2],'RN')
        else:
            performer, performerG, initiator = get_performer_by_mail(argv[2],'TN')
        mail_person=performer
        List_BS_with_describe =getParam(argv[1])
        if performerG=='error':
            reply_send(argv[2], 'Пользователь', 'Автозаведение работ', 'Email не зарегистрирован!')
            sys.exit()
        if not List_BS_with_describe:
            reply_send(argv[2], 'Пользователь', 'Автозаведение работ', 'Не удалось распознать сообщение.')
            sys.exit()
    except:
        print('Ошибка распознавания в письме')
        reply_send(argv[2], 'Пользователь', 'Автозаведение работ', 'Ошибка распознавания текста в письме!')
        sys.exit()
    options = webdriver.ChromeOptions()
    ...
    for BS,describeW in List_BS_with_describe:
        ...
        if remedyLogin(driver)!='получилось':
            driver_quit(driver)
        ...
        addInfo = ''
        opercontext=''
        supervisor = 'Оперативный дежурный ОКС СРД ФО'
        days = 1
        typeNE = 'БС'
        ...
        if 'уитс' in argv[3].lower():
            ...
            elif 'удалить' in describeW.lower():
                impactS = 'Нет влияния на сервис абонентам в зоне действия СЭ.'
                klassificator = ['14', '02', '07']
                days = 2
                typeNE = 'БС'
                addInfo = addInfo + " Работу по удалению из СУ инициировал " + performer + '.'
        ...
        if 'дэ' in argv[3].lower():
            klassificator = ['15', '12', '01']

        # по просьбе эксплуатации, если в теме письма содержится ДЭ
        # то заводится работа на классификатор эксплуатации
        if 'avr' in argv[3].lower():
        ...
        print(text)
        work=remedy_set_param(driver,typeW=typeW,
        typeS='Данные + Голос',
        impactS=impactS,
        addInfo=addInfo,
        ...
        if work:
            text=text + "\nЗаведена работа № %s" %work
            text=text+"\n"


            print(text+"\nВремя выполнения программы: %s секунд" % (time.time() - start_time))
            reply_send(argv[2],mail_person,'Автозаведение работ',text)
            save_cookies(driver, pathname)
    driver_quit(driver)
